...
def print_best_worst_vertical(
        ...
        metrics,
        prec=10,
        param_setting_renaming="default",
        color="equalParamColor",
        study_folder=None,
        label_prefix=""):
    """
    inputs:
    ...
    for metric in metrics:
        ...
        if not "time" in metric:
            param_list = ["training_time"] + param_list
            if "test" in metric:  # add validation metric to table
                param_list = ["final_val_" + metric.split("_")[1]] + param_list
            elif "val" in metric:  # add test metric to table
                param_list = ["test_" + metric.split("_")[2]] + param_list
        min_max = "min"  # partial headline for first iteration
        # treat the case where loss functions need to be compared without access to accuracy information:
        ...
        for top_n_list in min_max_lists[metric]:
            ...
            for param in param_list:
                new_param = rename_param(param, param_setting_renaming)
                # add first column content = parameter names
                line = f"{new_param:{max_param_length}}"
            ...
            print("\\hline\n")
            print(
                f"\\caption{{{best_worst} settings regarding \\textit{{{print_metric}}} for the {dataset} dataset}}")
            if label_prefix != "":
                print(
...
def set_print_filepath(metric, min_max, study_folder):
    """
    ...
    filename, dataset = get_filename(metric, min_max, study_folder)
...
def get_filename(metric, min_max, study_folder):
    """
